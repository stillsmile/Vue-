...
import { getToken } from '@/utils/auth' // get token from cookie //缓存中获取cookie
import getPageTitle from '@/utils/get-page-title'

NProgress.configure({ showSpinner: false }) // NProgress Configuration  //进程配置

const whiteList = ['/login', '/auth-redirect'] // no redirect whitelist  //重定向白名单，此列中的url不再进行重定向

router.beforeEach(async(to, from, next) => {
  // NProgress.js提供页面加载进度条效果，当页面打开加载时，
  // 在页面顶部会出现进度条加载动画。NProgress.js是轻量级的进度条组件，使用简便，可以很方便集成到单页面应用中。
  ...
  if (hasToken) {
    if (to.path === '/login') {
      ...
      if (hasRoles) {
        ...
        try {
          ...
          // generate accessible routes map based on roles //根据角色生成可访问路由映射
    ...
    if (whiteList.indexOf(to.path) !== -1) {
      // in the free login whitelist, go directly   //在免登录白名单，直接去
      next()
    } else {
      // other pages that do not have permission to access are redirected to the login page.  //有访问权限的其他页面被重定向到登录页面。
      next(`/login?redirect=${to.path}`)
      NProgress.done()
    }
  }
